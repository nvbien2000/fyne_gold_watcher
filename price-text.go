...
func (app *Config) getPriceText() (*canvas.Text, *canvas.Text, *canvas.Text) {
	...
	if err != nil {
		...
		if gold.GoldPrice < gold.GoldClose {
			displayColor = color.NRGBA{R: 180, G: 0, B: 0, A: 255}
		}

		openTxt := fmt.Sprintf("Open: $%.4f %s", gold.GoldClose, currency)
		currText := fmt.Sprintf("Current: $%.4f %s", gold.GoldPrice, currency)
		changeTxt := fmt.Sprintf("Change: $%.4f %s", gold.GoldChange, currency)

		open = canvas.NewText(openTxt, nil)
		current = canvas.NewText(currText, displayColor)
		change = canvas.NewText(changeTxt, displayColor)
	}

	open.Alignment = fyne.TextAlignLeading
	current.Alignment = fyne.TextAlignCenter
	change.Alignment = fyne.TextAlignCenter
